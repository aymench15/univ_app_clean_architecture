...
class DBHelper extends Equatable {
      ...
      onCreate: (db, version) async {
        await db.execute('''
          CREATE TABLE ${AppStrings.table_name} (
            ...
            ${AppStrings.uknown_fields} TEXT,
            ${AppStrings.module_name} TEXT,
            ${AppStrings.loc} TEXT,
            ${AppStrings.uknown_fields1} TEXT,
            ${AppStrings.uknown_fields1_2} TEXT,
  ...
  Future<int> insertNameGrp(String name, String id) async {
    ...
    return await db.insert(
        AppStrings.table_grp, {AppStrings.name: name, AppStrings.id: id});
  }
      ...
      for (final row in data) {
        ...
        await txn.insert(
          ...
          TimeTableDB(
            ...
            field: row[4],
            prof_lastName: row[5],
            prof_firstName: row[6],
            uknown_fields: row[7],
            module_name: row[8],
            loc: row[9],
            uknown_fields1: row[10],
            uknown_fields1_2: row[11],
            week_days: row[12],
            time_slot: row[13],
            uknown_fields2: row[14],
            uknown_fields3: row[15],
            uknown_fields4: row[16],
            uknown_fields5: row[17],
            sub_group: row[18],
            online: row[19],
            biweekly: row[20],
          ).toMap(),
